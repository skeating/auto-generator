...
class BaseCppFile(BaseFile.BaseFile):
    ...
    def expandAttributes(self, attributes):
        for i in range(0, len(attributes)):
            capName = strFunctions.cap(attributes[i]['name'])
    ...
    def writeCppNsBegin(self):
        self.skipLine(2)
        self.writeLine('{0}_CPP_NAMESPACE_BEGIN'
                       .format(self.libraryName.upper()))
        self.skipLine(2)

    def writeCppNsEnd(self):
        self.skipLine(2)
        self.writeLine('{0}_CPP_NAMESPACE_END'
                       .format(self.libraryName.upper()))
        self.skipLine(2)
