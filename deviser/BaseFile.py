...
class BaseFile:
    ...
    def writeLine(self, line):
        tabs = ''
        for i in range(0, int(self.numTabs)):
            tabs = tabs + '  '
    ...
    def addFileHeader(self):
        ...
        self.writeCommentLine('<!-----------------------------------------'
                              '---------------------------------')
        self.writeCommentLine('This file is part of libSBML.  Please visit '
                              'http://sbml.org for more')
        self.writeCommentLine('information about SBML, and the latest '
                              'version of libSBML.')
        self.writeBlankCommentLine()
        self.writeCommentLine('Copyright (C) 2013-2014 jointly by the '
                              'following organizations:')
        self.writeCommentLine('    1. California Institute of Technology, '
                              'Pasadena, CA, USA')
        self.writeCommentLine('    2. EMBL European Bioinformatics Institute '
                              '(EMBL-EBI), Hinxton, UK')
        self.writeCommentLine('    3. University of Heidelberg, Heidelberg, '
                              'Germany')
        self.writeBlankCommentLine()
        self.writeCommentLine('Copyright (C) 2009-2013 jointly by the '
                              'following organizations:')
        self.writeCommentLine('    1. California Institute of Technology, '
                              'Pasadena, CA, USA')
        self.writeCommentLine('    2. EMBL European Bioinformatics Institute '
                              '(EMBL-EBI), Hinxton, UK')
        self.writeBlankCommentLine()
        self.writeCommentLine('Copyright (C) 2006-2008 by the California '
                              'Institute of Technology,')
        self.writeCommentLine('    Pasadena, CA, USA ')
        self.writeBlankCommentLine()
        self.writeCommentLine('Copyright (C) 2002-2005 jointly by the '
                              'following organizations:')
        self.writeCommentLine('    1. California Institute of Technology, '
                              'Pasadena, CA, USA')
        self.writeCommentLine('    2. Japan Science and Technology Agency, '
                              'Japan')
        self.writeBlankCommentLine()
        self.writeCommentLine('This library is free software; you can '
                              'redistribute it and/or modify it')
        self.writeCommentLine('under the terms of the GNU Lesser General '
                              'Public License as published by')
        self.writeCommentLine('the Free Software Foundation.  A copy of the '
                              'license agreement is provided')
        self.writeCommentLine('in the file named "LICENSE.txt" included with '
                              'this software distribution and')
        self.writeCommentLine('also available online as http://sbml.org'
